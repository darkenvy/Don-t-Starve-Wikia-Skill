...
function scrape(url, item, evaluate) {
  return page.open(url + encodeURIComponent(item) + '&limit=1&namespaces=0%2C14', function(status) {
    var data = page.evaluate(evaluate);
    console.log('writing...', item + ': ' + data);
    fs.write(path, item + ': ' + data + ',\n', 'a');
    // phantom.exit(data);
  });
}
...
var evalGetIDs = function() {
  return JSON.parse(document.body.innerText).items[0].id;
}

var url = 'http://dontstarve.wikia.com/api/v1/Search/List/?query=';

// scrape(url, 'Mushroom Planter' ,evalGetIDs);
var current = 0;
setInterval(function() {
  if(current < phrase.length) {
    console.log('++ on: ' + current + '/' + phrase.length);
    scrape(url, phrase[current], evalGetIDs);
    current += 1;
  }
}, 2000)